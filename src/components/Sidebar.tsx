import { MoreVertical, ChevronLast, ChevronFirst, ShieldClose } from "lucide-react"
...
export function SidebarItem({ icon, text, active = false, alert = false }: SidebarItemProps) {
  ...
  return (
    ...
    >
      ...
      {alert && (
        <div
          className={`absolute right-2 w-2 h-2 rounded bg-gray-500 ${
            expanded ? "" : "top-2"
          }`}
        />
      )}
      ...
      {!expanded && (
        <div
          className={`absolute left-full rounded-md px-2 py-1 ml-6 bg-indigo-100 text-black text-sm invisible opacity-20 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}
        >
          {text}
        </div>
      )}
